...
int howManyBits(int x) {
  int sign = x >> 31, tmin = 1 << 31;
  int flag = (!x) + !(~x);
  int bits = 0, b16, b8, b4, b2, b1;
  b16 = (!(((x & (tmin >> 15)) >> 16) ^ sign)) << 4;
  bits += b16;
  x <<= b16;
  b8 = (!(((x & (tmin >> 7)) >> 24) ^ sign)) << 3;
  bits += b8;
  x <<= b8;
  b4 = (!(((x & (tmin >> 3)) >> 28) ^ sign)) << 2;
  bits += b4;
  x <<= b4;
  b2 = (!(((x & (tmin >> 1)) >> 30) ^ sign)) << 1;
  bits += b2;
  x <<= b2;
  b1 = !(((x & tmin) >> 31) ^ sign);
  bits += b1;
